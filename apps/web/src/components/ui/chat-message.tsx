        ...
        return (
          <div className={cn("flex flex-col", isUser ? "items-end" : "items-start")} key={`text-${index}`}>
            <div className={cn(chatBubbleVariants({ isUser, animation }))}>
              ...
              {actions ? (
                <div className="mt-1 flex justify-end space-x-1 rounded-lg border border-gray-200 bg-white p-1 text-gray-500">
                  {actions}
                </div>
              ) : null}
            </div>
      ...
      >
        <div className="flex items-center p-2">
          <CollapsibleTrigger className="flex items-center gap-2 text-muted-foreground text-sm hover:text-foreground">
            <ChevronRight className="h-4 w-4 transition-transform group-data-[open]:rotate-90" />
            <span>Thinking</span>
          </CollapsibleTrigger>
        </div>
