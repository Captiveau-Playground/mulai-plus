...
import { Download, Loader2, Maximize, Minus, Plus, ScanSearch } from "lucide-react";
import MindElixir, {
  type MindElixirData,
...
export const MindMap = forwardRef<MindMapRef, MindMapProps>(function MindMap(
  ...
  useEffect(() => {
    ...
    const initialData = toData(initialDataRef.current) || MindElixir.new("Mind Map");
    const themeToUse = initialData.theme || getTheme(resolvedThemeRef.current === "dark", monochrome);
    ...
    const options: Options = {
      ...
      alignment: "nodes",
      theme: themeToUse,
      markdown: markdownRef.current ? (text, obj) => markdownRef.current?.(text, obj) ?? text : undefined,
      imageProxy: imageProxyRef.current ? (url) => imageProxyRef.current?.(url) ?? url : undefined,
    };
  ...
  }, [direction, contextMenu, keypress, locale, overflowHidden, monochrome, readonly, fit, compact]);
  ...
  useEffect(() => {
    if (!mindRef.current || !isLoaded) return;
    mindRef.current.markdown = markdown ? (text, obj) => markdown(text, obj) : undefined;
    mindRef.current.imageProxy = imageProxy ? (url) => imageProxy(url) : undefined;
      ...
      {showZoom && (
        <>
          <button
            ...
            className="flex size-8 items-center justify-center rounded-md border border-border/50 bg-background/95 shadow-lg backdrop-blur-md transition-colors hover:bg-accent"
            aria-label="Zoom in"
          >
