...
// Calculate the color intensity based on bar height (pure)
...
// Draw a single bar of the visualizer (pure)
function drawBar(
...
export function AudioVisualizer({ stream, isRecording, onClick }: AudioVisualizerProps) {
  ...
  // Cleanup: stop animation loop and close audio context
  ...
  }, [stream, isRecording, startVisualization, cleanup]);
  ...
  return (
    // biome-ignore lint/a11y/noStaticElementInteractions: click-to-finish-recording overlay (visual only)
    // biome-ignore lint/a11y/useKeyWithClickEvents: recording overlay is visual-only (pointer gesture)
    <div
      ref={containerRef}
