    ...
    (message: Message) => ({
      actions: onRateResponse ? (
        <>
          ...
          >
            <ThumbsUp className={cn("h-4 w-4", message.feedback === "up" && "fill-current")} />
          </Button>
          <Button
            size="icon"
            variant="ghost"
          ...
          >
            <ThumbsDown className={cn("h-4 w-4", message.feedback === "down" && "fill-current")} />
          </Button>
        </>
      ) : (
        <CopyButton content={message.content} copyMessage="Copied response to clipboard!" />
  ...
  return (
    <ChatContainer className={className}>
      {isEmpty && append && suggestions ? (
        <PromptSuggestions label={suggestionsLabel} append={append} suggestions={suggestions} />
      ) : null}
...
export const ChatContainer = forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => {
    return <div ref={ref} className={cn("grid h-full max-h-full w-full grid-rows-[1fr_auto]", className)} {...props} />;
  },
);
