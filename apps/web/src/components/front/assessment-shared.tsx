...
export type AssessmentTheme = "violet" | "teal" | "default";

const THEME: Record<AssessmentTheme, { label: string; badge: string; border: string; check: string }> = {
  violet: {
...
export function AssessmentPricingSection({ theme = "default" }: { theme?: AssessmentTheme } = {}) {
  ...
  return (
    <section aria-label="Registrasi" className="mx-auto max-w-7xl bg-white px-4 py-14 sm:px-6 lg:px-8">
      ...
      <div className="mt-10 grid gap-5 md:grid-cols-2">
        {/* B2C */}
        <div className={cn("flex flex-col rounded-[2rem] border-2 bg-white p-7 shadow-sm", th.border)}>
          <div className="flex items-center gap-2">
            <span className={cn("rounded-full px-3 py-1 font-bold font-manrope text-[11px]", th.badge)}>
              UNTUK DIRI SENDIRI
            </span>
          ...
          <ul className="mt-4 space-y-2">
            {["Tanpa biaya, tanpa kartu", "Hasil instan + AI summary", "Laporan PDF", "Bisa diulang kapan saja"].map(
              (item) => (
                <li key={item} className="flex items-center gap-2 font-manrope text-gray-600 text-sm">
                  <CheckCircle2 className={cn("h-4 w-4 shrink-0", th.check)} /> {item}
                </li>
              ),
            )}
          </ul>
          ...
          <div className="relative flex flex-1 flex-col">
            ...
            <ul className="mt-4 space-y-2">
              {[
                ...
                <li key={item} className="flex items-center gap-2 font-manrope text-sm text-white/80">
                  <CheckCircle2 className="h-4 w-4 shrink-0 text-amber-300" /> {item}
                </li>
              ))}
            </ul>
            <div className="flex-1" />
          ...
          return (
            <div key={i} className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
              ...
              >
                ...
                <ChevronDown
                  className={cn("h-5 w-5 shrink-0 transition-transform duration-200", th.check, isOpen && "rotate-180")}
                />
              </button>
              {isOpen && (
