                  ...
                  },
                  {
                    ...
                    value:
                      analytics.totalPrograms > 0
                        ? Math.round(analytics.totalParticipants / analytics.totalPrograms)
                        : 0,
...
function BatchStatusCard({ analytics }: { analytics: any }) {
  const batchData = [
    { status: "upcoming", count: Math.max(1, Math.round(analytics.totalBatches * 0.15)), color: "bg-amber-500" },
    { status: "open", count: Math.max(1, Math.round(analytics.activeBatches * 0.4)), color: "bg-blue-500" },
    {
      status: "running",
      count: Math.max(1, analytics.activeBatches - Math.round(analytics.activeBatches * 0.4)),
      color: "bg-mentor-teal",
    },
    {
      status: "completed",
      count: Math.max(0, analytics.totalBatches - analytics.activeBatches - Math.round(analytics.totalBatches * 0.15)),
      color: "bg-gray-400",
    },
  ].filter((b) => b.count > 0);
          ...
          <div key={item.status} className="flex items-center gap-3 px-4 py-3">
            ...
            <div className="h-2 w-20 overflow-hidden rounded-full bg-gray-100">
              <div
                ...
                style={{ width: `${totalFromData > 0 ? (item.count / totalFromData) * 100 : 0}%` }}
              />
            </div>
          </div>
        ))}
