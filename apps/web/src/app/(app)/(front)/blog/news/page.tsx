...
function ArticleCard({ article, index }: { article: ArticleItem; index: number }) {
  return (
    ...
    >
      <Link href={`/blog/news/${article.slug}`} className="block">
...
export default function ArticlesPage() {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    ...orpc.cms.articles.public.list.queryOptions({
      input: { type: "news", categorySlug, limit: PAGE_SIZE, offset: page * PAGE_SIZE },
    }),
  });

  const articles = ((data?.data ?? []) as ArticleItem[]).filter((a) => a.type === "news");
