 ...
 * The db client is injected explicitly; the `@mulai-plus/db` import above is
 * TYPE-ONLY (erased at build — no `pg` runtime dependency). Schema imports are
 * safe: pure table definitions from `drizzle-orm/pg-core`, no DB connection.
 */
...
export const getRoles = async (client: typeof db) => {
  try {
    const roles = await client.select().from(schema.role);
 ...
 */
export const getAdminUserIds = async (client: typeof db) => {
  try {
    const admins = await client.select({ id: schema.user.id }).from(schema.user).where(eq(schema.user.role, "admin"));
